...
def acquire_data():
 ...
 # --- Medición de jitter ---
    ...
    jitter_std = (sum((x - media_intervalo) ** 2 for x in intervalos) / len(intervalos)) ** 0.5
    print(f"Jitter (desviación estándar de intervalos): {jitter_std:.2f} us")

    with open("jitter.txt", "w") as f:
        f.write("Intervalos(us)\n")
        for intervalo in intervalos:
            f.write(f"{intervalo}\n")
        f.write(f"\nJitter promedio (std dev): {jitter_std:.2f} us\n")
